...
- proxy: log a warning when falling back to prebuilt Java rules because no `RULES` were found in config (env var, parameter store, secret manager, GCS, or S3). This fallback will be removed in v0.7; rules must be explicitly configured.
