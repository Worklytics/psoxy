...
- `aws`: least-privileged provisioner policy (`psoxy-constants` `aws_least_privileged_policy`) adds tagging permissions needed when provider `default_tags` is set — notably `logs:ListTagsForResource` / `logs:TagResource` / `logs:UntagResource` (current CloudWatch Logs tagging APIs), `s3:PutObjectTagging` (proxy JAR upload when building from source), `lambda:ListTags` / `lambda:UntagResource`, and IAM `ListRoleTags` / `ListPolicyTags` / `ListOpenIDConnectProviderTags`. Docs call out the requirement.
