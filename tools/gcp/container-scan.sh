...
if [ -z "$INSTANCE_NAME" ]; then
  printf "${ERR}Instance name not provided. Exiting.${NC}\n"
  printf "$USAGE\n"
  exit 1
fi
...
printf "Looking up container image for ${INFO}${INSTANCE_NAME}${NC}...\n"
...
if [ -z "$VERSION" ]; then
  printf "${ERR}No container image found for ${INSTANCE_NAME}. Exiting.${NC}\n"
  printf "Make sure the Cloud Run service exists and has been deployed at least once.\n"
  printf "Expected artifact name: ${INFO}${ARTIFACT_NAME}${NC}\n"
  exit 1
fi

printf "Found image version: ${INFO}${VERSION}${NC}\n"
printf "Initiating container scan...\n"
printf "Results will be available in the GCP Console: ${INFO}https://console.cloud.google.com/artifacts/docker/$PROJECT_ID/$REGION/gcf-artifacts/$ARTIFACT_NAME?project=$PROJECT_ID${NC}\n\n"
...
if [ $SCAN_EXIT_CODE -ne 0 ]; then
  ...
  printf "  ${INFO}gcloud artifacts docker images describe ${REGION}-docker.pkg.dev/${PROJECT_ID}/gcf-artifacts/${ARTIFACT_NAME}@${VERSION}${NC}\n"
  exit $SCAN_EXIT_CODE
fi

printf "${SUCCESS}Container scan initiated successfully.${NC}\n"
