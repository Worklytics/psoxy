...
if ! gh pr view $PR_NUMBER >/dev/null 2>&1; then
  printf "PR ${INFO}${PR_NUMBER}${NC} does not exist. Exiting.\n"
  exit 1
fi
...
if gh release view $RELEASE_NUMBER >/dev/null 2>&1; then
  printf "Release ${INFO}${RELEASE_NUMBER}${NC} already exists. Exiting.\n"
  exit 1
fi

# check if git tag for release exists
if git rev-parse "$RELEASE_NUMBER" >/dev/null 2>&1; then
  printf "Git tag ${INFO}${RELEASE_NUMBER}${NC} already exists. Exiting.\n"
  exit 1
fi
...
if [ "$PR_STATUS" != "OPEN" ]; then
  printf "PR ${INFO}${PR_NUMBER}${NC} is not open. Continuing with cutting release from ${INFO}main${NC}, presuming it's already merged.\n"
else
  # check if PR is mergeable
  PR_MERGEABLE=$(gh pr view $PR_NUMBER --json mergeable | jq -r '.mergeable')
  if [ "$PR_MERGEABLE" != "MERGEABLE" ]; then
    printf "${ERR}PR ${PR_NUMBER} is not mergeable. Exiting.${NC}\n"
    exit 1
  fi
  ...
  if [ "$PR_CHECKS_PASSED" != "true" ]; then
    printf "${ERR}PR ${PR_NUMBER} does not have all status checks passing. Exiting.${NC}\n"
  ...
  printf "Merging PR ${INFO}${PR_NUMBER}${NC} to main ...\n"
  gh pr merge $PR_NUMBER --delete-branch --squash
fi

# ensure `main` up-to-date with origin
printf "Ensuring local ${INFO}main${NC} branch is up-to-date with origin ...\n"
...
printf "Tagging ${INFO}${RELEASE_NUMBER}${NC} ...\n"
git tag $RELEASE_NUMBER
git push origin $RELEASE_NUMBER

printf "Creating release ${INFO}${RELEASE_NUMBER}${NC} in GitHub ...\n"
