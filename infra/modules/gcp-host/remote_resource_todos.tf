...
output "remote_resource_opennlp_todo" {
  ...
  value = local.needs_opennlp_model_upload ? trimspace(<<-EOT
	...
	That command uses this deployment's artifacts bucket and shared prefix (`${local.shared_resource_path}`). It requires `curl` and `gsutil` with permission to write objects under that prefix.
