...
variable "vpc_config" {
  type = object({
    network = string # Local name or self-link of the VPC network resource on which to configure direct VPC egress
    subnet  = string # Local name or self-link of the VPC subnet resource on which to configure direct VPC egress
  })
