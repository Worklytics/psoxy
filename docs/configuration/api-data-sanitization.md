...
`<query-parameter-schemas> ::= "- queryParamSchemas: " <query-parameter-schema-map>`
...
- `type` :
  - `string` - no additional validation beyond other constraints below
  - `integer` - value must consist of digits only (no decimal point)
  - `number` - value must be parseable as a decimal number
- `format` :
  - `reversible-pseudonym` - value MUST be a reversible pseudonym generated by the proxy
  - `pseudonym` - alias for `reversible-pseudonym`
...
- `anyOf` - a list of parameter schemas; if any of these match, the value is valid

For query parameters only:

- `required` - if `true`, the query parameter must be present on the request

Absent query parameters and null values are valid unless `required: true` is set. Empty string values are validated according to the schema constraints.
