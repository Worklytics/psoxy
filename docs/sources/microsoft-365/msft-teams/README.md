...
Path parameters: `{userId}` / `{teamId}` / `{channelId}` / `{chatId}` are Entra/Teams GUIDs from `GET /v1.0/users`, `GET /v1.0/teams`, `GET .../allChannels`, and `GET .../chats`. `{meetingId}` is an onlineMeeting id from `GET /v1.0/users/{userId}/onlineMeetings`. `{reportId}` is an attendance report id from `GET .../attendanceReports`. `{callId}` / `{callRecordId}` are GUIDs from `GET /v1.0/communications/calls/{callId}` and `GET /v1.0/communications/callRecords` (`{callRecordId}` must be UUID-shaped). `{joinWebUrl}` is the online-meeting join URL used as the `JoinWebUrl` filter.
