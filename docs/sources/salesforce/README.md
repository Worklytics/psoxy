...
- `salesforce_example_account_id`: An Account Id (`{ANY_ACCOUNT_ID}`) used only in example test calls (`GET .../composite/sobjects/Account?ids={ANY_ACCOUNT_ID}` and `POST /services/data/v64.0/composite`).
