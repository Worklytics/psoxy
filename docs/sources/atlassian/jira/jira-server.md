...
Example API call path parameters: `{ISSUE_ID}` is a Jira issue key or id (`connector_settings.jira_example_issue_id`), e.g. `ETV-12`. `{API_VERSION}` in the rules is `2` or `latest` (both are allowed).
