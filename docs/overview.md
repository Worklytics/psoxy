...
5. **Client IP allowlisting** Optionally, restrict who may call proxy instances to a set of IP addresses or CIDR blocks. On **AWS**, Terraform can enforce this at **IAM** (assume-role `aws:SourceIp` conditions) and in the **proxy** (per-request checks). On **GCP**, enforcement is **application-level only** in the shipped modules; optional Cloud Armor in front of a load balancer is a separate network control. See [Client IP Allowlisting](configuration/ip-allowlisting.md). 
