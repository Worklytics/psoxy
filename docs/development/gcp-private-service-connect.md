...
2. **Network ingress (Cloud Armor)** — optional. When the client service dials out from known static egress IPs, attach Cloud Armor on an external Application Load Balancer (ALB). Set `external_api_alb` on `gcp-host` (or BYO via `api_connector_external_lb_host`); see [GCP External Application Load Balancer (ALB) + Cloud Armor](gcp-external-alb.md). You can also attach [Cloud Run ingress with Cloud Armor](https://cloud.google.com/run/docs/securing/cloud-armor) manually:
