...
public class ClientCredentialsGrantTokenRequestBuilder
    ...
    void setJWTCustomHeaders(JsonWebSignature.Header header) {
        String configuredValue = secretStore.getSecretOrError(ConfigProperty.PRIVATE_KEY_ID);
    ...
    private Map<String, String> buildClientSecretPayload() {
        ...
        data.put("client_secret",
            secretStore.getSecretOrError(ConfigProperty.CLIENT_SECRET));
    ...
    PrivateKey getPrivateKey() throws IOException {
        ConfigService.ConfigValueWithMetadata value = secretStore.getSecretWithMetadata(ConfigProperty.PRIVATE_KEY)
    ...
    public List<String> validateConfigValues() {
        ...
        if (getCredentialsType() == CredentialFlowType.CLIENT_SECRET) {
            if (StringUtils.isBlank(secretStore.getSecretOrError(ConfigProperty.CLIENT_SECRET))) {
                errors.add("Blank CLIENT_SECRET in secret store");
            }
        } else {
            if (StringUtils.isBlank(secretStore.getSecretOrError(ConfigProperty.PRIVATE_KEY_ID))) {
                errors.add("Blank PRIVATE_KEY_ID in secret store");
            }
            if (StringUtils.isBlank(secretStore.getSecretOrError(ConfigProperty.PRIVATE_KEY))) {
